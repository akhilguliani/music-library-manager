...
| Waveform Cache | `~/.vdj_manager/waveforms.db` | Waveform peak data for player |

All caches use file mtime + size for invalidation. If a file is modified, its cached results are automatically discarded.
...
| Mood | `Tags/@User2` | Hashtags | `"#happy #uplifting #summer"` |
...
Online mood uses a tiered lookup: Last.fm track tags -> Last.fm artist tags -> MusicBrainz genres -> local model fallback. All online lookups use exponential backoff retry for transient network errors.
