...
class TestGenerateStylesheet:
    ...
    def test_table_item_hover_and_selected(self):
        """QSS should include QTableView::item hover and selected states."""
        qss = generate_stylesheet(DARK_THEME)
        assert "QTableView::item:hover" in qss
        assert "QTableView::item:selected" in qss

    def test_list_widget_item_states(self):
        """QSS should include QListWidget::item hover and selected states."""
        qss = generate_stylesheet(DARK_THEME)
        assert "QListWidget::item:hover" in qss
        assert "QListWidget::item:selected" in qss

    def test_input_hover_states(self):
        """QSS should include hover states for input widgets."""
        qss = generate_stylesheet(DARK_THEME)
        assert "QLineEdit:hover" in qss
        assert "QSpinBox:hover" in qss
        assert "QDoubleSpinBox:hover" in qss
        assert "QComboBox:hover" in qss

    def test_input_disabled_states(self):
        """QSS should include disabled states for input widgets."""
        qss = generate_stylesheet(DARK_THEME)
        assert "QLineEdit:disabled" in qss
        assert "QSpinBox:disabled" in qss

    def test_scrollbar_pressed_state(self):
        """QSS should include pressed state for scrollbar handles."""
        qss = generate_stylesheet(DARK_THEME)
        assert "QScrollBar::handle:vertical:pressed" in qss
        assert "QScrollBar::handle:horizontal:pressed" in qss

    def test_splitter_handle_orientation(self):
        """QSS should include horizontal and vertical splitter handle styling."""
        qss = generate_stylesheet(DARK_THEME)
        assert "QSplitter::handle:horizontal" in qss
        assert "QSplitter::handle:vertical" in qss
        assert "QSplitter::handle:pressed" in qss

    def test_accent_button_pressed_and_disabled(self):
        """QSS should include pressed and disabled states for accent buttons."""
        qss = generate_stylesheet(DARK_THEME)
        assert 'QPushButton[class="accent"]:pressed' in qss
        assert 'QPushButton[class="accent"]:disabled' in qss

    def test_danger_button_hover_and_pressed(self):
        """QSS should include hover and pressed states for danger buttons."""
        qss = generate_stylesheet(DARK_THEME)
        assert 'QPushButton[class="danger"]:hover' in qss
        assert 'QPushButton[class="danger"]:pressed' in qss

    def test_tab_disabled_state(self):
        """QSS should include disabled state for tabs."""
        qss = generate_stylesheet(DARK_THEME)
        assert "QTabBar::tab:disabled" in qss

    def test_groupbox_title_uses_accent(self):
        """QGroupBox title should use accent color."""
        qss = generate_stylesheet(DARK_THEME)
        # The accent color should appear in the QGroupBox::title rule
        assert "QGroupBox::title" in qss
        assert DARK_THEME.accent in qss

    def test_slider_pressed_state(self):
        """QSS should include pressed state for slider handle."""
        qss = generate_stylesheet(DARK_THEME)
        assert "QSlider::handle:horizontal:pressed" in qss

    def test_menu_item_disabled(self):
        """QSS should include disabled state for menu items."""
        qss = generate_stylesheet(DARK_THEME)
        assert "QMenu::item:disabled" in qss

    def test_mini_player_pressed(self):
        """QSS should include pressed state for MiniPlayer buttons."""
        qss = generate_stylesheet(DARK_THEME)
        assert "MiniPlayer QPushButton:pressed" in qss

    def test_checkbox_hover_and_disabled(self):
        """QSS should include hover and disabled states for checkboxes."""
        qss = generate_stylesheet(DARK_THEME)
        assert "QCheckBox:hover" in qss
        assert "QCheckBox:disabled" in qss


class TestLightenDarken:
    """Tests for the _lighten and _darken color helper functions."""

    def test_lighten_returns_hex(self):
        """_lighten should return a 7-char hex color string."""
        result = _lighten("#000000", 0.5)
        assert result.startswith("#")
        assert len(result) == 7

    def test_darken_returns_hex(self):
        """_darken should return a 7-char hex color string."""
        result = _darken("#ffffff", 0.5)
        assert result.startswith("#")
        assert len(result) == 7

    def test_lighten_black(self):
        """Lightening black by 0.5 should produce mid-gray."""
        result = _lighten("#000000", 0.5)
        # Each channel: 0 + (255 - 0) * 0.5 = 127 = 0x7f
        assert result == "#7f7f7f"

    def test_darken_white(self):
        """Darkening white by 0.5 should produce mid-gray."""
        result = _darken("#ffffff", 0.5)
        # Each channel: 255 * (1 - 0.5) = 127 = 0x7f
        assert result == "#7f7f7f"

    def test_lighten_zero_factor_unchanged(self):
        """Lightening with factor 0 should return the same color."""
        color = "#5588cc"
        result = _lighten(color, 0.0)
        assert result == color

    def test_darken_zero_factor_unchanged(self):
        """Darkening with factor 0 should return the same color."""
        color = "#5588cc"
        result = _darken(color, 0.0)
        assert result == color

    def test_lighten_full_factor_white(self):
        """Lightening with factor 1.0 should produce white."""
        result = _lighten("#5588cc", 1.0)
        assert result == "#ffffff"

    def test_darken_full_factor_black(self):
        """Darkening with factor 1.0 should produce black."""
        result = _darken("#5588cc", 1.0)
        assert result == "#000000"

    def test_lighten_short_hex(self):
        """_lighten should handle 3-character hex shorthand."""
        result = _lighten("#fff", 0.0)
        assert result == "#ffffff"

    def test_darken_short_hex(self):
        """_darken should handle 3-character hex shorthand."""
        result = _darken("#fff", 0.0)
        assert result == "#ffffff"

    def test_lighten_makes_brighter(self):
        """Lightened color should have higher channel values."""
        original = "#445566"
        lighter = _lighten(original, 0.3)
        # Parse channels
        orig_r = int(original[1:3], 16)
        light_r = int(lighter[1:3], 16)
        assert light_r > orig_r

    def test_darken_makes_darker(self):
        """Darkened color should have lower channel values."""
        original = "#aabbcc"
        darker = _darken(original, 0.3)
        orig_r = int(original[1:3], 16)
        dark_r = int(darker[1:3], 16)
